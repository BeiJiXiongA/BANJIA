...
REGULAREXPRESSION(PhoneNumerRegularExpression, @"^1(3[0-9]|5[0-35-9]|8[0-35-9]|10|7[70])\\d{8}$")

REGULAREXPRESSION(EmailRegularExpression, @"[A-Z0-9a-z\\._%+-]+@([A-Za-z0-9-]+\\.)+[A-Za-z]{2,4}")
//REGULAREXPRESSION(EmailRegularExpression, @"")

//REGULAREXPRESSION(EmailRegularExpression, @"^(\\w)+(\\.\\w+)*@(\\w)+((\\.\\w+)+)$")
...
//    [self initPlist];
    ...
    UIColor *commonLinkColor = RGB(51, 186, 209, 1);
