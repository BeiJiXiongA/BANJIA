...
- (void)drawRect:(CGRect)rect 
{
    ...
    //x，y轴方向移动
	CGContextTranslateCTM(context, 10.0, self.bounds.size.height);/*self.bounds.size.height*/
...
//    CGContextTranslateCTM(context, - ceill(self.bounds.size.width) + self.frame.size.width-10, - ceill(self.bounds.size.height) + self.frame.size.height-topSpace);//加8是稍微调整一下位置，让字体完全现实，有时候y，j下面一点点会被遮盖
//	CGContextSetTextPosition(context, 0.0, 25.0); /*ceill(self.bounds.size.height) + 8*/
    //在离屏上绘制line
