...
//	CTLineDraw(line, context);
    ...
    //设置文字绘画的起点坐标。由于前面沿x轴翻转了（上面那条边）所以要移动到与此位置相同，也可以只改变CGContextSetTextPosition函数y的坐标，效果是一样的只是意义不一样
    CGContextTranslateCTM(context, - ceill(self.bounds.size.width) + self.frame.size.width-0, - ceill(self.bounds.size.height) + self.frame.size.height-topSpace);//加8是稍微调整一下位置，让字体完全现实，有时候y，j下面一点点会被遮盖
